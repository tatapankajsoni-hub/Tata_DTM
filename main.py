...
def health():
    try:
        with SessionLocal() as s:
            workshop_count = s.scalar(select(func.count()).select_from(Workshop)) or 0
            driver_count = s.scalar(select(func.count()).select_from(Driver)) or 0
        return {
            'ok': True,
            ...
            'integration': 'FastAPI + shared PostgreSQL'
        }
